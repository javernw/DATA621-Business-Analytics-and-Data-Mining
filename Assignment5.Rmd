...
#### Skewness in Data
```{r, fig.height=6, fig.width=9, message=FALSE, warning=FALSE, fig.align='center'}
...
A function was created to handle negative values with in the dataset. This will later help when applying the negative binomial models to the data.
...
New Variable added `BoundSulfurDioxide` which is:
...
The negative values are converted to absolute values. Below is a display of the *correlation* among these values. 
...
Correlation of negative values
```{r}
...
In this section we will build models to learn the data which we can eventually use to predict on the test set.
...
Below, the Poisson, Negative Binomial, and Multiple Linear Regression models have been compared to select the model with the lowest AIC.
...
```{r}
wine_eval2 <- wine_eval
wine_eval2$STARS <- as.factor(wine_eval2$STARS)
...
Prediction from optimal GLM model
```{r}
prob <- predict(p_mod2, wine_eval[,-2], type='response')
wine_eval$TARGET <- prob
wine_eval %>% head(10) %>% as_tibble()
write.csv(wine_eval, "wine_predictions.csv", row.names = FALSE)
```

Although not required, let's see how the linear model did when predicting the target values.

Prediction from optimal multiple linear regression model
```{r}
prob2 <- predict(lm_mod2, wine_eval2[,-2], interval ='prediction')
wine_eval2$TARGET <- prob2[,1]
wine_eval2 %>% head(10) %>% as_tibble()
write.csv(wine_eval, "wine_predictions2.csv", row.names = FALSE)
```


## Appendices

Source code: [GITHUB](https://github.com/javernw/DATA621-Business-Analytics-and-Data-Mining/blob/master/Assignment5.Rmd)


Predictions:

[Poission](https://github.com/javernw/DATA621-Business-Analytics-and-Data-Mining/blob/master/wine_predictions.csv)

[Linear](https://github.com/javernw/DATA621-Business-Analytics-and-Data-Mining/blob/master/wine_predictions2.csv)
